...
<head>
    ...
    <script type="text/javascript">   
		...
		viewer.onBeginExportReport = function (args) {
			args.fileName = "form-cetak";
		}
		
        // Load and show report
        var dataSet = new Stimulsoft.System.Data.DataSet("DataSet");
		dataSet.readJson({!! $anggota !!});

		var report = new Stimulsoft.Report.StiReport();
        report.loadFile("{{ asset('plugins/stimulsoft/reports/pkmf/' . $cetak->template) }}");
		...
		report.dictionary.variables.getByName("var_tempat").valueObject="{{ $tempat }}";
		report.dictionary.variables.getByName("var_tanggal").valueObject="{{ $tanggal }}";
		report.dictionary.variables.getByName("var_coach").valueObject="{{ $seminar->coach }}";
		report.dictionary.variables.getByName("var_penguji").valueObject="{{ $seminar->penguji }}";
		report.dictionary.variables.getByName("var_tt_jabatan").valueObject="{{ $tt->paraf2_jabatan }}";
		report.dictionary.variables.getByName("var_tt_nama").valueObject="{{ $tt->paraf2_nama }}";
		report.dictionary.variables.getByName("var_ruang").valueObject="{{ $ruang }}";
