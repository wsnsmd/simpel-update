    ...
    <div class="content">
        ...
        <div class="block block-rounded block-bordered block-fx-shadow block-themed">
            <div class="block-header block-header-default">
                <h3 class="block-title">Pengguna - Tambah Data</h3>
            </div>
            <div class="block-content block-content-full border-top">
                <form class="mb-2" action="{{ route('backend.pengguna.store') }}" method="POST" enctype="multipart/form-data" autocomplete="off">
                    @csrf
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control{{ $errors->has('nama') ? ' is-invalid' : '' }}" id="nama" name="nama" placeholder="Nama..." value="{{ old('nama') }}" required>
                            @if ($errors->has('nama'))
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" id="email" name="email" placeholder="Email..." value="{{ old('email') }}" required>
                            @if ($errors->has('email'))
                    ...
                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label text-right" for="password">Password <span class="text-danger">*</span></label>
                        <div class="col-sm-10">
                            <input type="password" class="form-control{{ $errors->has('password') ? ' is-invalid' : '' }}" id="password" name="password" placeholder="Password..." value="" required>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <select class="js-select2 form-control{{ $errors->has('instansi') ? ' is-invalid' : '' }}" id="instansi" name="instansi" style="width: 100%" required>
                                ...
                                <option value="{{ $i->id }}" {{ old('instansi') == $i->id ? "selected":"" }}>{{ $i->nama }}</option>
                                @endforeach
                            </select>
