    ...
    <div class="content">
        ...
        <div class="block block-rounded block-bordered block-fx-shadow block-themed">
            <div class="block-header block-header-default">
                <h3 class="block-title">Agama - Edit Data</h3>
            </div>
            <div class="block-content block-content-full">
                <form class="mb-2" action="{{ route('backend.master.agama.update', $agama->id) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label text-right" for="id">ID <span class="text-danger">*</span></label>
                        <div class="col-sm-3">
                            <input type="text" class="form-control{{ $errors->has('id') ? ' is-invalid' : '' }}" id="id" name="id" placeholder="ID..." value="{{ $agama->id }}">
                            @if ($errors->has('id'))
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control{{ $errors->has('nama') ? ' is-invalid' : '' }}" id="nama" name="nama" placeholder="Agama..." value="{{ $agama->nama }}">
                            @if ($errors->has('nama'))
