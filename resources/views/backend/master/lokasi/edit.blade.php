    ...
    <div class="content">
        ...
        <div class="block block-rounded block-bordered block-fx-shadow block-themed">
            <div class="block-header block-header-default">
                <h3 class="block-title">Lokasi - Edit Data</h3>
            </div>
            <div class="block-content block-content-full">
                <form class="mb-2" action="{{ route('backend.master.lokasi.update', $lokasi->id) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label text-right" for="nama">Nama Lokasi <span class="text-danger">*</span></label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control{{ $errors->has('nama') ? ' is-invalid' : '' }}" id="nama" name="nama" placeholder="Nama Lokasi..." value="{{ $lokasi->nama }}">
                            @if ($errors->has('nama'))
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control{{ $errors->has('ket') ? ' is-invalid' : '' }}" id="ket" name="ket" placeholder="Keterangan..." value="{{ $lokasi->ket }}">
                            @if ($errors->has('ket'))
