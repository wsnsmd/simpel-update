                            ...
                            @foreach ($templates as $i => $t)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if ($t->deskripsi)
                                            <small
                                                class="text-muted d-block">{{ \Illuminate\Support\Str::limit($t->deskripsi, 60) }}</small>
                                        @endif
                                    </td>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <button type="button" class="btn btn-sm btn-warning" data-toggle="modal"
                                            data-target="#modalEdit{{ $t->id }}" title="Edit">
                                            <i class="fa fa-pencil-alt"></i>
                                        </button>
                                        <form id="form-clone-{{ $t->id }}"
                                            action="{{ route('backend.diklat.nilai.template.clone', $t->id) }}" method="POST"
                                            class="d-inline">
                                            @csrf
                                        ...
                                        <form id="form-hapus-{{ $t->id }}"
                                            action="{{ route('backend.diklat.nilai.template.destroy', $t->id) }}" method="POST"
                                            class="d-inline">
                                            @csrf @method('DELETE')
                                            <button type="button" class="btn btn-sm btn-danger"
                                                onclick="hapusTemplate({{ $t->id }}, '{{ addslashes($t->nama) }}')" title="Hapus">
