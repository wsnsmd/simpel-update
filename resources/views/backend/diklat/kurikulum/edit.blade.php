    ...
    <div class="content">
        ...
        <div class="block block-rounded block-bordered block-fx-shadow block-themed">
            ...
            <div class="block-content block-content-full border-top">
                <form class="mb-2" action="{{ route('backend.diklat.kurikulum.update', $kurikulum->id) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label text-right" for="nama">Nama Kurikulum <span class="text-danger">*</span></label>
                        <div class="col-sm-9">
                            <input type="text" class="form-control{{ $errors->has('nama') ? ' is-invalid' : '' }}" id="nama" name="nama" placeholder="Nama Kurikulum..." value="{{ $kurikulum->nama }}">
                            @if ($errors->has('nama'))
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <select class="form-control{{ $errors->has('j_diklat') ? ' is-invalid' : '' }}" id="j_diklat" name="j_diklat">
                                ...
                                <option value="{{ $jd->id }}" {{ $kurikulum->diklat_jenis_id == $jd->id ? "selected":"" }}>{{ $jd->nama }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <select class="form-control{{ $errors->has('j_belajar') ? ' is-invalid' : '' }}" id="j_belajar" name="j_belajar">
                                ...
                                <option value="1" {{ $kurikulum->jenis_belajar == 1 ? "selected":"" }}>Klasikal Penuh</option>
                                <option value="2" {{ $kurikulum->jenis_belajar == 2 ? "selected":"" }}>Blended Learning</option>
                                <option value="3" {{ $kurikulum->jenis_belajar == 3 ? "selected":"" }}>E-Learning</option>
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control{{ $errors->has('jp') ? ' is-invalid' : '' }}" id="jp" name="jp" placeholder="Total JP..." value="{{ $kurikulum->total_jp }}">
                            @if ($errors->has('jp'))
